...
export class DBIDataExpertAgent extends BaseAgent<Env, AgentState> {
  ...
  async run(requestId: string, payload: SearchRequest): Promise<void> {
    ...
    // Optimized: Only iterate once over the selected datasets.
    // Sequential execution is maintained to respect early-exit logic and prevent potential race conditions.
